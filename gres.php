...
{ //--------- ADD PC ------------
    ...
    if ($xip <= 254) {
        ...
        $ts = time();
        db_query(
            'INSERT INTO servers(id, name,     ip,    owner,  cpu, ram, lan, mm, bb, ads, dialer, auctions, bankhack, fw, mk, av, ids, ips, rh, sdk, trojan, cc, lmupd, country, points, la, di, dt, lrh) VALUES('
    ...
    if ($pc['lmupd'] + 60 <= time()) {
        ...
        if ($pc['cc'] > $max) {
            if (isset($usr['syndikat'])) {
                ...
                if ($c !== false) {
                    $cc = $c['money'] + ($pc['cc'] - $max);
                    db_query(
                        'UPDATE syndikate SET money='.mysql_escape_string($cc).' WHERE id=\''.mysql_escape_string(
                            $usr['syndikat']
                        ).'\''
                    );
                }
        ...
        db_query(
            'UPDATE servers SET cc=\''.mysql_escape_string($pc['cc']).'\', lmupd=\''.time(
