  ...
  <article class="card span-6" id="syndikat">
    ...
    <?php if ($syndikat !== false) { ?>
      <ul class="muted" style="list-style:none; padding-left:0; margin:10px 0 0 0">
        ...
        <li>Geld: <?php echo number_format($syndikat['money'],0,',','.'); ?> CC</li>
      </ul>
    <?php } else { ?>
        ...
        if (isavailb($id, $tmppc) === true) {
            ...
            if ($cnt1 < UPGRADE_QUEUE_LENGTH && ($pc[$id] + $itemcnt) < itemmaxval($id)) {
                if ($pc['cryptocoins'] >= $inf['c']) {
                    ...
                    db_query(
                        'UPDATE `servers` SET `cryptocoins`=`cryptocoins`-'.mysql_escape_string(
                            $inf['c']
                        ).' WHERE `id`=\''.mysql_escape_string($pcid).'\''
                    );
                    db_query(
...
#$list='';
        ...
        if ($ext) {
            ...
            $sql = db_query(
                'SELECT id,name,ip,country,points,cryptocoins,rh,lrh FROM servers WHERE owner='.mysql_escape_string(
                    $usr['id']
                ).$ord.';'
            );
        }
...
createlayout_bottom();
                ...
                case 'syndikat':
                    echo '<b>Hiermit werden '.$cryptocoins.' CryptoCoins an den Syndikat '.$recip['code'].' ('.$recip['name'].') &uuml;berwiesen.</b><br />';
                    $c = GetCountry('id', $pc['country']);
                    $country = $c['name'];
                    $out = $c['out'];
                    ...
                    if ($rest > 0) {
                        ...
                        echo 'Da der Betrag sehr gering ist, werden keine Geb&uuml;hren erhoben. Der Syndikat '.$recip['code'].' erh&auml;lt <b>'.$cryptocoins.' CryptoCoins</b>.';
                        $fin = $cryptocoins;
                    }
                    break;
            }
...
#print_r($dat);
            db_query(
                'UPDATE servers SET cryptocoins=\''.mysql_escape_string($pc['cryptocoins']).'\' WHERE id='.mysql_escape_string(
                    $pcid
                )
            );
            if ($dat[0] == 'user') {
                ...
                db_query(
                    'UPDATE servers SET cryptocoins=cryptocoins+'.mysql_escape_string($dat[3]).' WHERE id=\''.mysql_escape_string(
                        $recip['id']
                    ).'\';'
                );
