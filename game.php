            ...
            if ($syndikat['tax'] > 0) {
                ...
                if ($pc['cc'] > 0) {
                    ...
                    $info .= infobox(
                        'Fehler',
                        'important',
                        'Du hast auf deinem ersten PC 10.47.'.$pc['ip'].' ('.$pc['name'].') nicht mehr gen&uuml;gend CryptoCoins um den Syndikat-Mitgliedsbeitrag von '.$syndikat['tax'].' CryptoCoins zu bezahlen.'
                    );
            ...
            if ($pc['cc'] - $c >= 0) {
                ...
                db_query(
                    'UPDATE servers SET '.mysql_escape_string($id).'='.mysql_escape_string(
                        $pc[$id]
                    ).', cc='.mysql_escape_string($pc['cc']).' WHERE id=\''.mysql_escape_string($pcid).'\''
                );
        ...
        if (isavailb($id, $tmppc) === true) {
            ...
            if ($cnt1 < UPGRADE_QUEUE_LENGTH && ($pc[$id] + $itemcnt) < itemmaxval($id)) {
                if ($pc['cc'] >= $inf['c']) {
                    ...
                    db_query(
                        'UPDATE `servers` SET `cc`=`cc`-'.mysql_escape_string(
                            $inf['c']
                        ).' WHERE `id`=\''.mysql_escape_string($pcid).'\''
                    );
                    db_query(
...
// Platz machen
    ...
    case 'servers': // -------------------------------- Servers --------------------------------
        ...
        setuserval('serverview_ext', ($ext ? 'yes' : 'no'));
...
#$list='';
        ...
        if ($ext) {
            ...
            $sql = db_query(
                'SELECT id,name,ip,country,points,cc,rh,lrh FROM servers WHERE owner='.mysql_escape_string(
                    $usr['id']
                ).$ord.';'
            );
        }
...
<td class="cc">'.$bucks.' CryptoCoins</td>';
            if ($ext) {
...
<p>';
            ...
            switch ($type) {
                case 'user':
                    ...
                    $text .= '<b>Hiermit werden '.$cc.' CryptoCoins an den Rechner 10.47.'.$recip['ip'].', der '.$ownerinfo.' geh&ouml;rt, &uuml;berwiesen.</b><br /><br />';
                    if ($pc['country'] == $recip['country']) {
                        ...
                        if ($rest > 0) {
                            ...
                            $text .= 'Da der Betrag sehr gering ist, werden keine Geb&uuml;hren erhoben. '.$recip_usr['name'].' erh&auml;lt <b>'.$cc.' CryptoCoins</b>.';
                            $fin = $cc;
                        }
...
createlayout_bottom();
                ...
                case 'syndikat':
                    echo '<b>Hiermit werden '.$cc.' CryptoCoins an den Syndikat '.$recip['code'].' ('.$recip['name'].') &uuml;berwiesen.</b><br />';
                    $c = GetCountry('id', $pc['country']);
                    $country = $c['name'];
                    $out = $c['out'];
                    $rest = $cc - $out;
                    if ($rest > 0) {
                        $fin = $rest;
                        echo 'Davon werden noch '.$out.' CryptoCoins als Ausfuhr-Geb&uuml;hr f&uuml;r '.$country.' abgezogen. Der Syndikat '.$recip['code'].' erh&auml;lt also noch <b>'.$rest.' CryptoCoins</b>';
                    } else {
                        echo 'Da der Betrag sehr gering ist, werden keine Geb&uuml;hren erhoben. Der Syndikat '.$recip['code'].' erh&auml;lt <b>'.$cc.' CryptoCoins</b>.';
                        $fin = $cc;
                    }
                    break;
            }
...
<?php
createlayout_bottom();
            file_put($DATADIR.'/tmp/transfer_'.$tcode.'.txt', $type.'|'.$recip['id'].'|'.$cc.'|'.$fin);
            db_query(
...
#print_r($dat);
            db_query(
                'UPDATE servers SET cc=\''.mysql_escape_string($pc['cc']).'\' WHERE id='.mysql_escape_string(
                    $pcid
                )
            );
            if ($dat[0] == 'user') {
                ...
                $recip['cc'] += $dat[3];
                db_query(
                    'UPDATE servers SET cc=cc+'.mysql_escape_string($dat[3]).' WHERE id=\''.mysql_escape_string(
                        $recip['id']
                    ).'\';'
                );
                ...
                $c['money'] += $dat[3];
                $c['events'] = nicetime4(
                    ).' [usr='.$usrid.']'.$usr['name'].'[/usr] spendet dem Syndikat '.$dat[3].' CryptoCoins.'.LF.$c['events'];
                db_query(
                    'UPDATE syndikate SET money=\''.mysql_escape_string($c['money']).'\',events=\''.mysql_escape_string(
                        $c['events']
                    ).'\' WHERE id='.mysql_escape_string($c['id'])
                );
                $msg = 'Dem Syndikat '.$c['code'].' wurden '.$dat['2'].' CryptoCoins &uuml;berwiesen!';
            }
            db_query(
