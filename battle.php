...
#echo 'gone='.$gone.'<br />';
            ...
            echo '<div class="error"><h3>Hinweis</h3><p>Du kannst erst wieder '.nicetime2(
                    $next,
                    true
                ).' angreifen!</p></div>';
        ...
        $v = $pc['mk'];
        $weapons = '';
        if (isavailh('scan', $pc) == true) {
            $weapons .= '<tr class="greytr2"><td nowrap="nowrap">REMOTE SCAN</td><td><tt>REMOTE SCAN</tt></td><td>Spioniert fremde Rechner aus</td><td>Waffe</td></tr>';
        }
        if (isavailh('trojan', $pc) == true) {
            $tEx = '<table style="font-size:8pt;" class="nomargin">';
            if (tisavail('defacement') == true) {
                $tEx .= '<tr><td nowrap="nowrap"><tt>TROJAN DEFACEMENT</tt></td><td>Du kannst die Beschreibung des gegnerischen Users &auml;ndern</td></tr>';
            }
            if (tisavail('transfer') == true) {
                $tEx .= '<tr><td nowrap="nowrap"><tt>TROJAN TRANSFER</tt></td><td>Klaut Geld</td></tr>';
            }
            if (tisavail('deactivate') == true) {
                $tEx .= '<tr><td nowrap="nowrap"><tt>TROJAN DEACTIVATE FIREWALL</tt></td><td>Deaktiviert die Firewall</td></tr>
      ...
      <tr><td nowrap="nowrap"><tt>TROJAN DEACTIVATE IDS</tt></td><td>Deaktiviert das IDS</td></tr>';
        ...
        if (isavailh('smash', $pc) == true) {
            $weapons .= '<tr class="greytr2"><td nowrap="nowrap">REMOTE SMASH [ITEM]</td><td nowrap><tt>REMOTE SMASH CPU</tt><br /> oder
...
<input type="reset" value="  Reset  " /> <input type=submit value=" Weiter " /></th></tr>
    ...
    case 'opc_submit': // --------------- OPC Submit -----------------------
        ...
        if (eregi('^10\\.47\\.([0-9]{2,3})\\.([0-9]{1,3})$', $target) == true) {
            ...
            if ($target === false) {
                $e .= 'Ung&uuml;ltige Ziel-Adresse: Der PC existiert nicht!<br />';
            }
        } else {
        ...
        if ($scan & isavailh('scan', $pc) == false) {
            $e .= 'Der Scan ist noch nicht verf&uuml;gbar!<br />';
        }
        if ($trojan & isavailh('trojan', $pc) == false) {
            $e .= 'Der Trojaner ist noch nicht verf&uuml;gbar!<br />';
        }
        if ($block & isavailh('block', $pc) == false) {
            $e .= '<tt>REMOTE BLOCK</tt> ist noch nicht verf&uuml;gbar!<br />';
        }
        if ($smash & isavailh('smash', $pc) == false) {
            $e .= '<tt>REMOTE SMASH</tt> ist noch nicht verf&uuml;gbar!<br />';
        }
        if ($hijack & isavailh('rh', $pc) == false) {
            $e .= '<tt>REMOTE HIJACK</tt> ist noch nicht verf&uuml;gbar!<br />';
        }

        if ($hijack & ($trojan || $scan || $smash || $block)) {
            $e .= 'Der Hijack kann nur alleine eingesetzt werden!<br />';
        }
        if ($trojan & ($smash || $block || $scan)) {
            $e .= 'Der Trojaner kann nur alleine eingesetzt werden!<br />';
        }
        if ($smash & ($trojan || $block || $scan)) {
            $e .= '<tt>REMOTE SMASH</tt> kann nur alleine eingesetzt werden!<br />';
        }
        if ($block & ($smash || $trojan || $scan)) {
            $e .= '<tt>REMOTE BLOCK</tt> kann nur alleine eingesetzt werden!<br />';
        }
        if ($smash & $opt == '') {
            $e .= '<tt>REMOTE SMASH</tt> erwartet einen Parameter!<br />';
        }
        if ($trojan & $opt == '') {
            $e .= 'Der Trojaner erwartet einen Parameter!<br />';
        }
        if (!($hijack || $trojan || $scan || $smash || $block)) {
            $e .= 'Du musst mindestens eine Waffe angeben!<br />';
        }

        if ($hijack && ($pc['lrh'] + REMOTE_HIJACK_DELAY > time() && $localhost == false)) {
            $e .= 'Du kannst immer nur einen <tt>REMOTE HIJACK</tt>-Angriff innerhalb von zwei Tagen ausf&uuml;hren!<br />';
        }
        ...
        if ($trojan) {
            switch ($opt) {
                case 'DEFACEMENT':
                    if (tisavail('defacement') == false) {
                        $e .= 'Defacement ist noch nicht verf&uuml;gbar!<br />';
                    }
                    break;
                case 'TRANSFER':
                    if (tisavail('transfer') == false) {
                        $e .= 'Geld klauen ist noch nicht verf&uuml;gbar!<br />';
                    }
                    break;
                case 'DEACTIVATE FIREWALL':
                    if (tisavail('deactivate') == false) {
                        $e .= 'Deaktivierung ist noch nicht verf&uuml;gbar!<br />';
                    }
                    break;
                case 'DEACTIVATE AV':
                    if (tisavail('deactivate') == false) {
...
# Durch Trojaner deaktivierte Items beachten!!
            ...
            if ($pc['cc'] - $cost < 0 && $country != $country2) {
                $e .= 'Nicht gen&uuml;gend CryptoCoins! Dieser Angriff w&uuml;rde '.$cost.' CryptoCoins kosten, du hast aber nur '.$pc['cc'].' CryptoCoins!<br />';
            }
        }
...
<ul><li><b>Der Angriff richtet sich gegen 10.47.'.$target['ip'].' ('.$target['name'].'). Dieser Rechner geh&ouml;rt ';
            ...
            if ($msg != '') {
                $text .= '<li><b>Kommentar:</b><br /><tt>'.nl2br($msg).'</tt></li>';
            }
            if ($scan) {
                $text .= '<li>Es wird <tt>REMOTE SCAN</tt> eingesetzt, um den Rechner auszuspionieren!</li>';
            }
            if ($trojan) {
                switch ($opt) {
                    case 'DEFACEMENT':
                        $s = 'die Beschreibung des Gegners zu &auml;ndern';
                        break;
                    case 'TRANSFER':
                        $s = 'Geld zu klauen';
                        break;
                    case 'DEACTIVATE FIREWALL':
                        $s = 'die Firewall zu deaktivieren';
                        break;
                    case 'DEACTIVATE AV':
                        $s = 'das Anti-Virus-Programm zu deaktivieren';
                        break;
                    case 'DEACTIVATE IDS':
                        $s = 'das IDS zu deaktivieren';
                        break;
                }
            ...
            if ($block) {
                $text .= '<li>Es wird <tt>REMOTE BLOCK</tt> eingesetzt, um den feindlichen Rechner zu blockieren.</li>';
            }
            if ($hijack) {
                $text .= '<li>Es wird <tt>REMOTE HIJACK</tt> eingesetzt.</li>';
            }
            if ($king) {
                $text .= '<li><br /><br />Der KING ist der KING!</li>';
            }
            $acode = randomx(16);
            file_put(
                $DATADIR.'/tmp/attack_'.$acode.'.txt',
                implode(
                    '|',
                    array($target['id'], $scan, $trojan, $smash, $block, $hijack, $opt, $msg, $cost, $pc['id'])
                )
            );
            db_query(
                'UPDATE users SET acode=\''.mysql_escape_string($acode).'\' WHERE id=\''.mysql_escape_string(
                    $usrid
                ).'\' LIMIT 1;'
            );
...
'.$text.'
    ...
    case 'pre_execute': // ---------------------- PRE_EXECUTE -----------------------
        ...
        list($target, $scan, $trojan, $smash, $block, $hijack, $opt, $msg, $cost, $pcidnew) = explode(
            '|',
            file_get($DATADIR.'/tmp/attack_'.$code.'.txt')
        );
        ...
        if ($cost > 0):
            $pc['cc'] -= $cost;
            db_query('UPDATE servers SET cc='.$pc['cc'].' WHERE id='.$pcid.';');
        endif;
...
<p><form><input type=button onclick="abortattack()" value=" Abbrechen "></form></p>
    ...
    case 'execute': // --------------------------------- EXECUTE ---------------------------------
        ...
        if (trim($msg) != '') {
            $msg = '<br />Der Angreifer hat folgende Nachricht f&uuml;r dich:<br /><tt>'.nl2br($msg).'</tt>';
        }
...
# Durch Trojaner deaktivierte Items beachten!!
        $s = $remote['di'];
        if ($s != '' && (int)$remote['dt'] > time()) {
            $remote[$s] = 0;
        }
        $s = $local['di'];
        if ($s != '' && (int)$local['dt'] > time()) {
            $local[$s] = 0;
        }
        ...
        if ($scan == 1) { // ------------ SCAN ------------
            ...
            if ($success == 0) {
                ...
                if ($noticed == 0) {
                    echo 'Du hattest aber Gl&uuml;ck und wurdest nicht erkannt!';
                    addsysmsg(
                        $remote['owner'],
                        'Auf deinen Rechner 10.47.'.$remote['ip'].' ('.$remote['name'].') wurde ein Spionage-Angriff ver&uuml;bt!<br />Er wurde abgeblockt!<br />Der Feind konnte nicht identifiziert werden!'.$msg
                    );
                } else {
                    echo 'Du konntest nicht anonym bleiben!';
                    addsysmsg(
                        $remote['owner'],
                        'Auf deinen Rechner 10.47.'.$remote['ip'].' ('.$remote['name'].') wurde ein Spionage-Angriff ver&uuml;bt!<br />Er konnte aber abgeblockt werden!<br />Der Feind wurde als [usr='.$usrid.']'.$usr['name'].'[/usr] identifiziert.'.$msg
                    );
                }
            } else {
                echo '<span style="color:green;"><b>Der Angriff war erfolgreich!</b></span><br />';
                if ($noticed == 0) {
                    echo 'Du wurdest au&szlig;erdem nicht bemerkt!';
                } else {
                    echo 'Du konntest aber nicht anonym bleiben!';
                    addsysmsg(
                        $remote['owner'],
                        'Auf deinen Rechner 10.47.'.$remote['ip'].' ('.$remote['name'].') wurde ein Spionage-Angriff ver&uuml;bt!<br />Er war erfolgreich!<br />Der Feind konnte als [usr='.$usrid.']'.$usr['name'].'[/usr] identifiziert werden!'.$msg
                    );
                }
    ...
    Geld: '.$remote['cc'].' CryptoCoins';
            ...
            function deactivate($key)
            {
                ...
                $success = $tmp['success'];
                $noticed = $tmp['noticed'];
                if ($success == 1 && $noticed == 0) {
                    $t = 90;
                } elseif ($success == 1 && $noticed == 1) {
                    $t = 45;
                }
                ...
                if ($success == 1) {
                    $t = time() + $t * 60;
                    echo '<span style="color:green;font-weight:bold;">Angriff erfolgreich! '.idtoname(
                            $key
                        ).' blockiert bis '.nicetime($t).'!</span><br />';
                    $s = 'Auf deinen PC 10.47.'.$remote['ip'].' ('.$remote['name'].') wurde ein erfolgreicher Angriff ver&uuml;bt: '.idtoname(
                            $key
                    ...
                    $s = 'Es wurde versucht '.idtoname(
                            $key
                        ).' auf deinem PC 10.47.'.$remote['ip'].' ('.$remote['name'].') zu blockieren. Der Angriff konnte aber abgeblockt werden!<br />';
                }
...
<br /><input type=submit value="Speichern">
                        ...
                        if ($noticed == 1) {
                            ...
                            addsysmsg(
                                $remote['owner'],
                                'Auf dich wurde ein erfolgreicher Defacement-Angriff ver&uuml;bt!<br />Der Feind konnte als [usr='.$usrid.']'.$usr['name'].'[/usr] identifiziert werden!'.$msg
                            );
                        } else {
                            echo 'Du wurdest bemerkt, aber nicht erkannt!';
                            addsysmsg(
                                $remote['owner'],
                                'Auf dich wurde ein erfolgreicher Defacement-Angriff ver&uuml;bt!<br />Der Feind konnte nicht identifiziert werden!'.$msg
                            );
                        }
                    } else {
                        ...
                        if ($noticed == 1) {
                            addsysmsg(
                                $remote['owner'],
                                'Es wurde erfolglos versucht, dich mit einem Defacement-Angriff zu schm&auml;hen!<br />Der Feind konnte als [usr='.$usrid.']'.$usr['name'].'[/usr] identifiziert werden!'.$msg
                            );
                            echo 'Du wurdest bemerkt und erkannt!';
                        } else {
                ...
                case 'TRANSFER': // ------------ TRANSFER ------------
                    ...
                    if ($remote2['lmupd'] + 60 <= time()) {
                        ...
                        db_query(
                            'UPDATE servers SET lmupd=\''.mysql_escape_string(
                                $remote2['lmupd']
                            ).'\', cc=\''.mysql_escape_string(
                                $remote2['cc']
                            ).'\' WHERE id='.mysql_escape_string($remote2['id']).';'
                        );
                        $remote['cc'] = $remote2['cc'];
                    }
                    ...
                    if ($success == 1) {
                        ...
                        $cc = floor($remote['cc'] / $quo);
                        if (getmaxbb($local) < $local['cc'] + $cc) {
                            $cc = getmaxbb($local) - $local['cc'];
                            if ($cc == 0) {
                        ...
                        $remote2['cc'] -= $cc;
                        $local['cc'] += $cc;
                        if ($local['cc'] > 0 && $remote2['cc'] > 0) {
                            db_query(
                                'UPDATE servers SET cc=\''.mysql_escape_string(
                                    $remote2['cc']
                                ).'\' WHERE id='.mysql_escape_string($remote2['id']).';'
                            );
                            db_query(
                        ...
                        echo '<span style="color:green;"><b>Der Angriff war erfolgreich! Du hast '.$cc.' CryptoCoins geklaut!</b></span><br /><br />';
                        $s = 'Von deinem Rechner 10.47.'.$remote['ip'].' ('.$remote['name'].') wurden '.$cc.' CryptoCoins geklaut!<br />';
                    } else {
                        ...
                        $s = 'Es wurde erfolglos versucht, Geld von deinem Rechner 10.47.'.mysql_escape_string(
                                $remote['ip']
                            ).' ('.mysql_escape_string($remote['name']).') zu klauen!<br />';
                    }
            ...
            function smash($key)
            {
                ...
                $s = 'Auf deinen PC 10.47.'.mysql_escape_string($remote['ip']).' ('.mysql_escape_string(
                        $remote['name']
                    ).') wurde ein SMASH-Angriff ver&uuml;bt! ';
                if ($success == 1) {
                    $min = 0;
                    if ($remote[$key] < 1) {
                        $min = 1;
                        ...
                        if ($key == 'cpu') {
                            if ($cpu_levels[$remote['cpu']] > $cpu_levels[0]) {
                                $newval = getlastcpulevel($remote['cpu']);
                            } else {
                                $min = 1;
                            }
                        } else {
                            if ($remote[$key] > 1) {
                                $newval = ($remote[$key] - 0.5);
                            } else {
                                $min = 1;
                            }
                        }
                    ...
                    if ($min == 1) {
                        $xs = idtoname(
                                $key
                            ).' konnte aber nicht weiter zerst&ouml;rt werden, da das niedriegste Level schon erreicht wurde!';
                        echo $xs.'</span><br />';
                        $s .= $xs.'<br />';
                    } else {
                        $remote2[$key] = $newval;
            ...
            $defend = getDefend(
                '$remote[\'av\']*2+$remote[\'fw\']*2+$remote[\'ids\']*2+$remote[\'cpu\']+$remote[\'ram\']/2'
            );
            $attack = getAttack(
                '$local[\'sdk\']+$local[\'mk\']+$local[\'cpu\']+$local[\'ram\']+$local[\'ips\']+$local[\'rh\']*2'
            );
            $tmp = getsuccess($attack, $defend, 20); # Toleranz 20
            $success = $tmp['success'];
            $noticed = $tmp['noticed'];
            $tmp = getsuccess($attack, $defend, 5); # Toleranz 5
            if ($tmp['success'] == 1 & $tmp['noticed'] == 1) {
                $success = 0;
                $noticed = 1;
            }
            if (mt_rand(0, 50) < 4) {
            ...
            if ($success == 0) {
                ...
                if ($noticed == 1) {
                    ...
                    $s .= 'Der Angreifer konnte anonym bleiben!';
                }
                addsysmsg($remote['owner'], $s.$msg);
