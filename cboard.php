...
switch ($action) {
    ...
    case 'addreply': // ------------------------ ADD REPLY ---------------------------------
        ...
        if ($text != '') {
            ...
            $sql = 'INSERT INTO cboards VALUES (\''.mysql_escape_string($cix).'\', \'0\', \''.mysql_escape_string(
                    $thread
...
function formatText($s)
{
    ...
    for ($i = 0; $i < count($a); $i++) {
        $text = str_replace(
            htmlentities($a[$i]['symbol']),
            '<img src="smilies/'.$a[$i]['file'].'" align=middle border=0>',
            $text
        );
    }
...
<td><textarea name="text" cols="50" rows="5" onkeyup="setCaret(this)" onmouseup="setCaret(this)" onchange="setCaret(this)">';
    ...
    $a = LoadSmilies();
    for ($i = 0; $i < count($a); $i++) {
        echo '<a href="javascript:smily(\''.htmlentities(
                $a[$i]['symbol']
            ).'\');"><img src="smilies/'.$a[$i]['file'].'" title="'.$a[$i]['symbol'].'" alt="Smilie" border="0" /></a> ';
    }
