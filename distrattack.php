...
';
            ...
            while ($item = mysql_fetch_assoc($r)) {
                ...
                if ($initusr !== false) {
                    ...
                    $result = db_query(
                        'SELECT * FROM da_participants WHERE relative=\''.mysql_escape_string($item['id']).'\''
                    );
...
';
    ...
    case 'create_submit': // -------------------------------- CREATE SUBMIT -------------------------------
        ...
        if ($e == '') {
            ...
            $cc = $country['in'] * 20;
            if ($pc['cc'] < $cc) {
                $e .= 'Du hast nicht gen&uuml;gend Geld, um die Einfuhr-Geb&uuml;hr von '.$cc.' CryptoCoins nach '.$country['name'].', dem Standort von 10.47.'.$target['ip'].', zu bezahlen!';
            } else {
                $code = randomx(10);
...
<tr>
<th>Kosten:</th>
<td>F&uuml;r die Einfuhr nach '.$country['name'].' fallen '.$cc.' CryptoCoins an.<br />
...
';
                file_put($DATADIR.'/tmp/da_'.$code.'.txt', $target['id'].'|'.$cc.'|'.$t);
            }
        }

        if ($e != '') {
            echo '<div class="error">'.LF.'<h3>Fehler!</h3>'.LF.'<p>'.$e.'</p>'.LF.'</div>'."\n";
        }
    ...
    case 'create_final': // -------------------------------- CREATE FINAL -------------------------------
        ...
        list($target, $cc, $item) = explode('|', file_get($fn));
        @unlink($fn);
        db_query(
        ...
        $id = mysql_insert_id();
        db_query(
            'INSERT INTO da_participants SET relative='.mysql_escape_string($id).', pc='.mysql_escape_string(
                $pc['id']
            ).', ip=\''.mysql_escape_string($pc['ip']).'\', owner='.mysql_escape_string(
                $usrid
...
#echo mysql_error();
        $pc['cc'] -= $cc;
        db_query(
            'UPDATE servers SET cc='.mysql_escape_string($pc['cc']).' WHERE id='.mysql_escape_string($pcid).';'
        );
    ...
    case 'join': // -------------------------------- JOIN -------------------------------
        ...
        if (isset($a['id']) == true && $a['syndikat'] == $syndikatid) {
            ...
            while ($x = mysql_fetch_assoc($sql)) {
                ...
                $result = db_query(
                    'SELECT relative FROM da_participants WHERE relative=\''.mysql_escape_string(
                        $a['id']
                    ).'\' AND pc=\''.mysql_escape_string($x['id']).'\';'
                );
                ...
                if ($x['lmupd'] + 60 <= time()) {
                    ...
                    db_query(
                        'UPDATE servers SET lmupd=\''.$x['lmupd'].'\', cc=\''.$x['cc'].'\' WHERE id='.mysql_escape_string(
                            $x['id']
                        ).';'
                    );
                }
        ...
        if (isset($a['id']) == true && $a['syndikat'] == $syndikatid) {
            ...
            while ($xpc = mysql_fetch_assoc($sql)) {
                ...
                $result = db_query(
                    'SELECT relative FROM da_participants WHERE relative=\''.mysql_escape_string(
                        $a['id']
                    ).'\' AND pc=\''.mysql_escape_string($xpc['id']).'\';'
                );
                if (mysql_num_rows($result) > 0) {
                    continue;
                }
                $creds -= 1000;
                db_query(
                    'UPDATE servers SET cc=\''.mysql_escape_string($creds).'\' WHERE id=\''.mysql_escape_string(
                        $xpc['id']
                    ).'\';'
                );
                db_query(
                    'INSERT INTO da_participants SET relative=\''.mysql_escape_string(
                        $a['id']
                    ).'\', pc=\''.mysql_escape_string($xpc['id']).'\', ip=\''.mysql_escape_string(
                        $xpc['ip']
                    ).'\', owner=\''.mysql_escape_string($usrid).'\', owner_name=\''.mysql_escape_string(
                        $usr['name']
                    ).'\';'
                );
            }
...
<table>';
        ...
        while ($x = mysql_fetch_assoc($sql)):
            ...
            if ($x['lmupd'] + 60 <= time()) {
                ...
                db_query(
                    'UPDATE servers SET lmupd=\''.mysql_escape_string($x['lmupd']).'\', cc=\''.mysql_escape_string(
                        $x['cc']
    ...
    case 'exec': // -------------------------------- EXECUTE -------------------------------
        ...
        if (isset($a['id']) == true && $a['syndikat'] == $syndikatid) {
            ...
            if ($total > 1) {
                ...
                $defend = $remote['cpu'] * count(
                        $item
                    ) + $remote['fw'] * 2 + $remote['ids'] + $remote['av'] / 2 + mt_rand(0, 8 * count($item));
                $dif = $attack - $defend;
...
############
                ...
                if ($dif < 10 || $remote[$a['item']] == $lv || ($a['item'] == 'cpu' && $remote[$a['item']] == array_search(
                            $lv,
                            $cpu_levels
                        ))
                ) {
                    ...
                    $msg = 'Die Distributed Attack auf 10.47.'.$remote['ip'].' (Besitzer: [usr='.$remote['owner'].']'.$name['name'].'[/usr]), an der du teilgenommen hast, war erfolgreich!!<br />'.idtoname(
                            $a['item']
                ...
                echo '<div class="error"><h3>Fehler</h3><p>Es m&uuml;ssen sich noch andere User dem Angriff anschlie&szlig;en!</p></div>';
            }
        }
