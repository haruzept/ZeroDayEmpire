...
if ($usr['bigacc'] != 'yes') {
    simple_message('Nur f&uuml;r User mit Extended Account!');
    exit;
}
...
<tr>';
    ...
    case 'add': //------------------------- ADD -------------------------------
        ...
        if ($u !== false) {
            ...
            $field = @mysql_result($r, 0, 'set'.$g);
            if (!$field) {
                db_query('INSERT INTO abooks VALUES(\''.$usrid.'\',\'\',\'\',\'\',\'\')');
                $r = db_query('SELECT * FROM abooks WHERE user=\''.$usrid.'\'');
                $field = @mysql_result($r, 0, 'set'.$g);
            }
            ...
            db_query(
                'UPDATE abooks SET set'.mysql_escape_string($g).'=\''.mysql_escape_string(
                    $field
...
<tr><td colspan="2" align="right"><input type="submit" value="Hinzuf&uuml;gen" /></td></tr>
        ...
        function admin_list($ix)
        {
            ...
            $s = @mysql_result(
                db_query(
                    'SELECT `set'.mysql_escape_string($ix).'` FROM abooks WHERE user='.mysql_escape_string($usrid)
                ),
                'set'.mysql_escape_string($ix)
            );
            if ($s != '') {
                $a = explode(',', $s);
                if (!(count($a) <= 1 && $a[0] == '')) {
...
<tr><th>User</th><th>Gruppe</th><th>L&ouml;schen?</th></tr>';
    ...
    case 'saveadmin': //------------------------- SAVE ADMIN -------------------------------
        ...
        db_query(
            'UPDATE abooks SET set1=\''.mysql_escape_string($b['set1']).'\', set2=\''.mysql_escape_string(
                $b['set2']
            ).'\', set3=\''.mysql_escape_string($b['set3']).'\', set4=\''.mysql_escape_string(
                $b['set4']
            ).'\' WHERE user='.mysql_escape_string($usrid)
        );
