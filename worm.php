...
switch ($action) {
    ...
    case 1: // Geld aus Syndikatkasse eines Syndikate mit mehr als einer Million CryptoCoins klauen
        ...
        db_query(
            ...
            ' cc from syndikat ' . mysqli_real_escape_string($dbcon, $victim['id']) . '\';'
        );
        break;
  ...
  bis '.nicetime($blocked).' blockiert!'
    ...
    case 3: // PC von aktivem User aus dem Mittelfeld der Rangliste CryptoCoins schenken
        ...
        #echo '<br>id='.$victim['id'];
        $vpc = @mysqli_fetch_assoc(
            db_query('SELECT id,ip,name,cc FROM servers WHERE owner=' . $victim['id'] . ' ORDER BY RAND() LIMIT 1;')
        );
        $plus = mt_rand(2000, 10000);
        $creds = $vpc['cc'] + $plus;
        db_query(
            'UPDATE servers SET cc=\'' . mysqli_real_escape_string($dbcon, $creds) . '\' WHERE id=' . mysqli_real_escape_string($dbcon, $vpc['id']) . ';'
        );
        addsysmsg(
            $victim['id'],
  ...
  die Summe von '.$plus.' CryptoCoins &uuml;berwiesen!'
        ...
        db_query(
            ...
            ' cc to server ' . mysqli_real_escape_string($dbcon, $vpc['id']) . '\';'
        );
        break;
