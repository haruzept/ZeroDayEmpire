...
$usr = getuser($usrid);
if ($usr === false || $usr['bigacc'] != 'yes') {
    exit;
}
if ($usr['enable_usrimg'] == 'no') {
    exit;
}
...
if (file_exists($imgfile) != true) {
    ...
    if (stristr($option, 'points') != false) {
        $w[1] = imagefontwidth(2) * strlen($usr['points'].' Punkte');
    }
    ...
    if (stristr($option, 'points') != false) {
        imagestring($hImg, 2, 20, $currenty, $usr['points'].' Punkte', $black);
        $currenty += 12;
    }
    if (stristr($option, 'ranking') != false) {
        imagestring($hImg, 2, 20, $currenty, 'Platz '.$usr['rank'], $black);
        $currenty += 12;
    }
